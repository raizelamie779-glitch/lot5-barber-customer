    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (currentMode === 'booking') {
                ...
                queueData.appointment_time = `${pgDate}T${pgTime}+08:00`; // Postgres will parse this as a valid timestamptz in local time
            }
            ...
            if (currentMode === 'walkin') {
                window.location.href = `../your_appointment_status/?queue_id=${queueId}`;
            } else {
                window.location.href = `../booking_confirmed_status/?queue_id=${queueId}`;
            }
...
async function loadAvailabilityForDate(dateObj) {
    ...
    // Use ISO format with local timezone offset (+08:00) to ensure accurate day boundaries
    const startOfDay = `${pgDate}T00:00:00+08:00`;
    const endOfDay = `${pgDate}T23:59:59+08:00`;
...
function renderTimeSlots(dateObj) {
    ...
    ALL_TIME_SLOTS.forEach(timeStr => {
        ...
        // For booked check, use local timezone (+08:00) comparison
        const localDate = new Date(`${pgDate}T${pgTime}+08:00`);
